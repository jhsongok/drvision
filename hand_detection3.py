...
while cap.isOpened():
    ...
    if result.multi_hand_landmarks is not None:
        rps_result = []  # 가위, 바위, 보 결과를 저장할 빈 리스트
        ...
        for res in result.multi_hand_landmarks:
            ...
            if idx in rps_gesture.keys():  # 가위, 바위, 보 중에 하나면 출력하기
                ...
                cv2.putText(img, text=gesture_name.upper(), org=(org[0], org[1]+20), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(0, 0, 255), thickness=2)

                rps_result.append({             # 가위, 바위, 보 결과를 리스트에 저장
                    'rps': rps_gesture[idx],
                    'org': org
                })
            ...
            if len(rps_result) >= 2:
                ...
                if winner is not None:
                    cv2.putText(img, text='Winner', org=(rps_result[winner]['org'][0], rps_result[winner]['org'][1] + 70), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1.5, color=(0, 255, 255), thickness=3)
                    cv2.putText(img, text=text, org=(140, 80), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2, color=(0, 0, 255), thickness=4)
